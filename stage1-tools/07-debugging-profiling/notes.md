...
🕷 **课堂增量：user/real 的定量解读 + 并行度不足的三步诊断流**（2026-09-07 授课）

**定量**：`user ÷ real ≈ 平均满载核数`。例：real 8s、user 15s → 平均只有 ~1.9 核在干活——若目标是 8 核满载，问"**其余核上的线程停在哪个函数上？**"（栈顶即答案）。

**三步诊断流（从粗到细）**：
1. `htop` 按 `H` 显示线程：几根核条在跳（排除线程数没开够）？稳定 100% = 真在算（并行度不足）；抖动 = 抢锁/同步
2. `pidstat -t -p <PID> 1`：每秒打印**每个线程**的 CPU 占用——"谁在干活、谁在陪跑"点名到人
3. 看闲线程的栈：`gdb -p <PID>` 后 `thread apply all bt`（拍所有线程栈）或 `perf record -g -p <PID>`

**病因对照表**（闲线程栈顶在哪，病就在哪）：

| 栈顶/热点 | 病因 | 处方 |
|-----------|------|------|
| `futex_wait` / `pthread_mutex_lock` | 锁竞争（临界区太粗） | 缩临界区/换原子/分桶 |
| `__kmpc_barrier`（OpenMP） | 负载不均，全队等最慢 | schedule(dynamic/guided) |
| 某个串行函数（IO/单核大循环） | **Amdahl**：主体没并行 | 并行化主体/换算法 |
| `read`/`write`/`fsync` | 等磁盘 | 数据分盘/缓存 |
